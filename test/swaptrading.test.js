...
test('Slot C and Slot A are compatible with each other', () => {
    const app = buildSwapApp();
    const result = app._checkSwapCompliance(baseRequest({ requester_slot_type: 'slotC', responder_slot_type: 'slotA', desired_slot_type: 'slotA' }));
    assert.equal(result.passed, true, `expected pass, got reasons: ${result.reasons.join(' | ')}`);
});

test('Slot D cannot trade with Slot A/B/C', () => {
    const app = buildSwapApp();
    const result = app._checkSwapCompliance(baseRequest({ requester_slot_type: 'slotD', responder_slot_type: 'slotA', desired_slot_type: 'slotD' }));
    assert.equal(result.passed, false);
...
test('Slot D can trade with another Slot D', () => {
    const app = buildSwapApp();
    const result = app._checkSwapCompliance(baseRequest({ requester_slot_type: 'slotD', responder_slot_type: 'slotD', desired_slot_type: 'slotD' }));
    assert.equal(result.passed, true, `expected pass, got reasons: ${result.reasons.join(' | ')}`);
});
...
test('a request with no desired slot type on record cannot be validated', () => {
    const app = buildSwapApp();
    const result = app._checkSwapCompliance(baseRequest({ desired_slot_type: null }));
    assert.equal(result.passed, false);
    assert.ok(result.reasons.some(r => r.includes('no desired slot type on record')));
});
...
test('multiple simultaneous failures are all reported, not just the first one found', () => {
    ...
    const result = app._checkSwapCompliance(baseRequest({
        ...
        desired_slot_type: 'slotA',                       // matches what's actually offered — keeps Rule 3 from firing here
